...
_DRV_LED_EXTERNAL_ volatile UINT8 aLedDisBuf[DRV_LED_TOTAL_LEDs];
_DRV_LED_EXTERNAL_ volatile DRV_LED_NUM_T   vLedPointBuf;
 ...
 *      2011.6.7        Panda.Xiong         Create/Update
 *****************************************************************************/
#define DRV_LED_SetLedData(_n, _p, _v)                                      \
    do {                                                                    \
        /* set point value */                                               \
        vLedPointBuf = ((vLedPointBuf & ~((DRV_LED_NUM_T)1 << (_n)))        \
                        | ((DRV_LED_NUM_T)((BOOL)(_p)) << (_n)));           \
                                                                            \
        /* set display data */                                              \
        aLedDisBuf[(_n)] = (_v);                                            \
    } while (0)
